...
namespace DG.Tweening
{
    ...
    public class DOTween
    {
        ...
        public static IDOTweenInit Init(bool? recycleAllByDefault = null, bool? useSafeMode = null, LogBehaviour? logBehaviour = null)
        {
            ...
            DOTweenSettings settings = null;//Resources.Load(DOTweenSettings.AssetName) as DOTweenSettings;
            ...
            for (int i = 0; i < totIterations; ++i) {
                if (i < totIterations - 1) {
                    ...
                    if (vectorBased) {
                        ...
                        Vector3 to = rndQuaternion * DG.Tweening.Core.Utils.Vector3FromAngle(ang, shakeMagnitude);
                        ...
                        if (ignoreZAxis) {
                            tos[i] = DG.Tweening.Core.Utils.Vector3FromAngle(ang, shakeMagnitude);
                        } else {
                            Quaternion rndQuaternion = Quaternion.AngleAxis(UnityEngine.Random.Range(-randomness, randomness), Vector3.up);
                            tos[i] = rndQuaternion * DG.Tweening.Core.Utils.Vector3FromAngle(ang, shakeMagnitude);
                        }
                        if (fadeOut) shakeMagnitude -= decayXTween;
                    }
                } else tos[i] = Vector3.zero;
